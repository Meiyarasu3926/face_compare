...
#             # Save as temporary file with specific naming
#             temp_path = os.path.join(self.config.TEMP_FOLDER, f"process_{os.urandom(8).hex()}.jpg")
...
#             logger.error(f"DeepFace ValueError: {ve}")
#             return None, "No face detected in the image"
#         except Exception as e:
#             logger.error(f"Error calculating embedding: {e}")
#             return None, str(e)
...
#             temp_file = os.path.join(self.config.TEMP_FOLDER, f"upload_{os.urandom(8).hex()}{suffix}")
#             # Read image bytes
...
#             cv2.imwrite(temp_file, img, [cv2.IMWRITE_JPEG_QUALITY, 85])
#             return temp_file
#         except Exception as e:
#             logger.error(f"Error saving upload file: {e}")
#             raise HTTPException(status_code=500, detail=str(e))
